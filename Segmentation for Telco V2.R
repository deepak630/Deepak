#Aet the direcotry
setwd("F:/BigData/BIG DATA(R+HADOOP)/Week-4/Day-2/")

getwd()

#Importing Data
telco<-read.csv("D:/Analytics/R/BA class 3+4/Factor & Cluster Analysis/Factor/telco_csv.csv")
...
mystats <- function(x) {
  nmiss<-sum(is.na(x)) #missing
  a <- x[!is.na(x)]    #not missing
  m <- mean(a)      
  n <- length(a)
  s <- sd(a)
  min <- min(a)
  pctls<-quantile(a,probs=c(0.01, 0.05,0.1,0.25,0.5,0.75,0.9,0.95,0.99)) # percentile operation which are not missing
    max <- max(a)
...
write.csv(diag_stats, "C:/business analYtix 36O/diag_stats.csv")
...
tabular(r1+r2+r3~ mean*c1+ mean*c2+ mean*c3,data=)
